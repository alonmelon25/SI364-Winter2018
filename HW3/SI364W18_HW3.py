...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/proaaronHW3"
## Provided:
...
#manager = Manager(app) # For debugging purposes
...
class Tweet(db.Model):
    ...
    def __repr__(self):
        return '<Tweet text {%r} (ID: {%r})>' % (self.text, self.id)
...
def index():
    ...
    ## And redirect to the list of all tweets
        tweet_exists = Tweet.query.filter_by(text = text, user_id = user.id).first()
        if tweet_exists:
    ...
    ## Redirect to the index page
        new_tweet = Tweet(text = text, User = user)
...
def see_all_tweets():
    ...
    for tweet in tweets:
        search = User.query.filter_by(id= tweet.user_id).first()
        all_tweets.append((tweet.text, search.username))
...
# Retrieve all Tweets
# Make text as a list and sort by len text
...
def longest_tweet():
    ...
    longest_tweet = Tweet.query.filter_by(text = sorted_tweet).first()
    user = User.query.filter_by(id = longest_tweet.user_id).first().username
    return render_template('longest_tweet.html', text= sorted_tweet, user= user)
